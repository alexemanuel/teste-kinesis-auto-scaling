...
resource aws_cloudwatch_metric_alarm kinesis_scale_up {
  ...
  alarm_actions             = [var.scaling_sns_topic_arn]
...
resource aws_cloudwatch_metric_alarm kinesis_scale_down {
  ...
  threshold                 = var.kinesis_stream.shard_count == 1 ? -1 : var.kinesis_scale_down_threshold        # Defined in scale.tf
  alarm_description         = "Stream throughput has gone below the scale down threshold"
  insufficient_data_actions = []
  alarm_actions             = [var.kinesis_scaling_sns_topic_arn]
  ...
  metric_query {
    ...
    expression = var.kinesis_scale_down_min_iter_age_mins
  }
  ...
  metric_query {
    ...
    expression = "(FILL(m3,0)/1000/60)*(${var.kinesis_scale_down_threshold}/s2)" # We want to block scaledowns when IterAge is > 60 mins, multiply IterAge so 60 mins = <alarmThreshold>
  }
